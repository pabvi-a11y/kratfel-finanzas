...
$pnl = [];
foreach ($pnlRaw as $r) { $g=$r['grupo_pnl']; $pnl[$g]['total']=($pnl[$g]['total']??0)+(float)$r['total']; }
$GRP = ['operativo'=>'Gasto operativo','distribucion'=>'Distribuciones a socios','impuestos'=>'Impuestos'];

// Reserva histórica (12 meses completos) reconstruida desde los retiros
...
$histRes = array_fill(0,12,0.0);
$acc = $saldo + ($consumo[$cur] ?? 0);   // reserva al cierre del mes anterior
$histRes[11] = $acc;
...
$data = ['saldo'=>$saldo,'asof'=>$asof,'vel'=>$vel['velocidad'],
  'histLabels'=>$histLabels,'histRes'=>array_map(fn($v)=>round($v),$histRes)];
...
.legend i.d{border-top-style:dashed;border-color:#7aa2ff}.legend i.z{border-color:var(--bad)}
</style></head>
<body>
...
<nav style="display:flex;gap:6px;margin-left:8px"><a href="/" style="color:#e8ecf7;background:#1e2540;padding:8px 14px;border-radius:10px;text-decoration:none;font-size:14px;font-weight:600">Dashboard</a><a href="/pnl.php" style="color:#9aa6c7;padding:8px 14px;text-decoration:none;font-size:14px;font-weight:600">P&amp;L</a><a href="/forecast.php" style="color:#9aa6c7;padding:8px 14px;text-decoration:none;font-size:14px;font-weight:600">Forecast</a><a href="/flujo.php" style="color:#9aa6c7;padding:8px 14px;text-decoration:none;font-size:14px;font-weight:600">Flujo</a></nav>
...
function render(){const s=scen.find(x=>x.id===cur),p=project(s);
 ...
 const labels=[...D.histLabels,...p.lbl];                  // histórico(12) + hoy + futuro
 const nh=D.histLabels.length;
 const solid=[...D.histRes, D.saldo, ...Array(p.lbl.length-1).fill(null)];
 const dashed=[...Array(nh).fill(null), ...p.val];          // empieza en 'hoy'(=saldo) y proyecta
